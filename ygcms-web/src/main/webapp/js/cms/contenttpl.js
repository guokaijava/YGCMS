var contenttpl = function(){
	...
	var rename = function(data){
		var dialog = $('<div class="modal fade"><div class="modal-dialog">'
		 ...
		 dialog.find('#save').on('click',function(e){
           ...
           $.post(baseUrl+'rename.koala?opath='+$("#curDir").val()+'/'+dialog.find("#snameID").val()+'&npath='+$("#curDir").val()+'/'+dialog.find("#nnameID").val()).done(function(result){
                if(result.success){
                     dialog.modal('hide');
                     var grid = $('#filegrid');
	...
	var xiugai = function(data){
		 ...
		 $.get('/pages/content/CmsFile-add.jsp').done(function(html){
			 ...
			 $.get( baseUrl+'/get.koala?path='+$("#curDir").val()+'/'+data).done(function(json){
                   json = json.data;
		 ...
		 dialog.find('#save').on('click', function(e){
             if(!Validator.Validate(dialog.find('form')[0],3))return;
             $.post(baseUrl+'addFile.koala?path='+$("#curDir").val()+'/'+dialog.find("#fname").val(),dialog.find('form').serialize()).done(function(result){
                  if(result.success){
                	  dialog.modal('hide');
                      var grid = $('#filegrid');
                      grid.data('koala.grid').refresh();
