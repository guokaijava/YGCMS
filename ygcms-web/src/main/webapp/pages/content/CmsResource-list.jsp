...
<div class="department-detail">
	<ul class="u-tree tree" id="resTree"
	...
	<div class="right-content" style="width: 70%;padding-top:5px;padding-right:0px;height:540px">
		<div id="resgrid" style="hight:540px">
			
		</div>
	</div>
	<div style="clear: both;"></div>
	<input id="rescurDir" type="hidden"/>
</div>
<script type="text/javascript"
	src="<c:url value='/js/cms/siteresource.js' />"></script>
<script type="text/javascript">
	...
	resource().getTree();
    	 ...
    	 var cols = [{
               ...
               sortName: 'fileName',render:function(rowdata,name,index){
               	...
               	if(!rowdata.directory){
                    ...
                    var str = "png,jpg,gif";
               	  var houzhui = rowdata.fileName.substring(rowdata.fileName.lastIndexOf('.')+1);
                  if(str.indexOf(houzhui)<0){
                	  h='<span class="glyphicon glyphicon-file" aria-hidden="true"><span>&nbsp;'+rowdata.fileName+'</span></span>'; 
                  }else{
                	  h='<span class="glyphicon glyphicon-picture" aria-hidden="true"><span alt="www">&nbsp;'+rowdata.fileName+'</span></span>';  
                  }
               	}
               	return h;
               }
               ...
               render:function(rowdata,name,index){
                   var h='<a href="javascript:resource().rename(\''+rowdata.fileName+'\')">重命名</a>';
                   var str = "png,jpg,gif";
               	   if(!rowdata.directory){
                       var houzhui = rowdata.fileName.substring(rowdata.fileName.lastIndexOf('.')+1);
                       if(str.indexOf(houzhui)<0){
                       	   h+='&nbsp;&nbsp;<a href="javascript:resource().xiugai(\''+rowdata.fileName+'\')">修改</a>';
                       }else{
                    	   //h+='&nbsp;&nbsp;<a href="javascript:resource().viewimage(\''+rowdata.fileName+'\')">查看</a>';
                       }
               	   }
            	   return h;
               }
     ...
     var addDir = function(grid){
  		 ...
  		 dialog.find('#save').on('click',{grid: grid}, function(e){
               ...
               $.post(baseUrl+'addDir.koala?path='+$("#rescurDir").val()+'/'+dialog.find("#dirnameID").val()).done(function(result){
                    if(result.success){
                         dialog.modal('hide');
      ...
      var addFile = function(grid){
   		 var dialog = $('<div class="modal fade"><div class="modal-dialog">'
   		        	...
   		        	+'<button type="button" class="btn btn-success" id="save">保存</button></div></div>'
   		        	+'</div></div>');
   		 $.get('/pages/content/CmsRes-add.jsp').done(function(html){
   			 dialog.modal({
   				 keyboard:false
   			 }).on({
   				 'hidden.bs.modal': function(){
   		 ...
   		 dialog.find('#save').on('click',{grid: grid}, function(e){
                if(!Validator.Validate(dialog.find('form')[0],3))return;
                $.post(baseUrl+'addFile.koala?path='+$("#rescurDir").val()+'/'+dialog.find("#fname").val()+dialog.find("#zhui").val(),dialog.find('form').serialize()).done(function(result){
                     if(result.success){
                          dialog.modal('hide');
                          e.data.grid.data('koala.grid').refresh();
                          e.data.grid.message({
