...
public class ShiroUser implements Serializable {
	...
	private Long getRoleIdByUserAccount() {
		InvokeResult result = getSecurityAccessFacade().findRolesByUserAccount(userAccount);
		if (result.isSuccess()) {
			List<RoleDTO> roles = (List<RoleDTO>) result.getData();
			if (!roles.isEmpty()) {
	...
	public List<CmsSiteDTO> getCmsSiteList() {
		Long roleId = getRoleIdByUserAccount();
		List<CmsSiteDTO> csdlist = new ArrayList<CmsSiteDTO>();
		if(roleId!=null&&!roleId.equals("暂未分配角色")){
			csdlist = securityAccessFacade.getSiteListByRoleId(roleId);
		}
		return csdlist;
	}
